...
export default function PublicQuotePage() {
  ...
  return (
    <PublicSite className="bg-[#F7F2EA] text-[#173F46]">
      <main className="flex-1 px-4 py-8 sm:py-14">
        <div className="mx-auto max-w-2xl min-w-0">
          <div className="mb-6 flex min-w-0 items-center gap-3"><div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-[#102E38] text-[#D6A85F]">CF</div><div className="min-w-0"><p className="text-xl font-semibold">CleanFixHarish</p><p className="text-sm text-[#746D65]">{q.privateQuote}</p></div></div>
          ...
          {quote && <Card className="overflow-hidden border-[#D8D0C6] shadow-sm">
            <CardHeader className="bg-[#102E38] text-white"><div className="flex min-w-0 flex-wrap items-center justify-between gap-3"><div className="min-w-0"><p className="text-sm text-white/70" dir="ltr">Quote #{quote.id}</p><CardTitle className="mt-1 break-words text-4xl" dir="ltr">{money(quote.quoted_total)}</CardTitle></div><Badge className="bg-[#F7F2EA] text-[#173F46]">{quote.status}</Badge></div></CardHeader>
            <CardContent className="space-y-6 p-5 sm:p-8">
              ...
              <div className="public-grid grid min-w-0 gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-[#F0EAE1] p-4"><p className="text-xs uppercase tracking-wider text-[#786F65]">{q.deposit}</p><p className="mt-1 text-xl font-semibold" dir="ltr">{money(quote.deposit_required)}</p></div><div className="rounded-2xl bg-[#F0EAE1] p-4"><p className="text-xs uppercase tracking-wider text-[#786F65]">{q.validUntil}</p><p className="mt-1 font-medium">{new Date(quote.expires_at).toLocaleString(lang === 'he' ? 'he-IL' : 'en-IL')}</p></div></div>
              {error && <p className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}
              {quote.status === 'published' && <div className="public-grid grid min-w-0 gap-3 sm:grid-cols-2"><Button disabled={deciding} onClick={() => decide('accept')} className="h-12 bg-[#174E57]"><CheckCircle2 className="me-2 h-5 w-5"/>{q.accept}</Button><Button disabled={deciding} onClick={() => decide('decline')} variant="outline" className="h-12"><XCircle className="me-2 h-5 w-5"/>{q.decline}</Button></div>}
