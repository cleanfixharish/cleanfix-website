...
type Section = 'overview' | 'assistant' | 'leads' | 'whatsapp' | 'jobs' | 'providers' | 'services' | 'pricing' | 'content' | 'followups' | 'platforms' | 'internal';
...
const viewerLockedSections: Section[] = ['assistant', 'whatsapp', 'pricing', 'content', 'followups', 'platforms', 'internal'];
...
export default function AdminPage() {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#F2EDE5] text-[#243538]">
    ...
    <main className="relative z-10 min-w-0 px-3 py-4 sm:p-6 lg:ml-64 lg:p-8">
      ...
      {!isViewer && section === 'pricing' && <PricingWorkspace leads={leads}/>}
...
type PricingReference = { id:number; observation_key:string; category:string; sub_service:string; geography:string; min_price?:number; max_price?:number; vat_status:string; validation_status:string; eligible_for_estimate:boolean; source:{publisher:string;url:string} };
type PricingEstimate = { id:number; service_description:string; geography:string; suggested_min?:number; suggested_max?:number; customer_min?:number; customer_max?:number; provider_budget?:number; status:string; disclaimer:string };
type LocalPricingEvidence = { id:number; evidence_kind:string; category:string; sub_service:string; geography:string; customer_price?:number; provider_amount?:number; scope_notes:string; status:string };
type LocalPricingBenchmark = { category:string; sub_service:string; geography:string; sample_count:number; customer_average?:number; provider_average?:number; margin_average?:number; ready_for_guidance:boolean; minimum_samples:number };

function PricingWorkspace({ leads }: { leads: DashboardLead[] }) {
  const [references,setReferences]=useState<PricingReference[]>([]); const [estimates,setEstimates]=useState<PricingEstimate[]>([]); const [localEvidence,setLocalEvidence]=useState<LocalPricingEvidence[]>([]); const [benchmarks,setBenchmarks]=useState<LocalPricingBenchmark[]>([]);
  const [selected,setSelected]=useState(''); const [leadId,setLeadId]=useState('none'); const [description,setDescription]=useState(''); const [geography,setGeography]=useState('Harish');
  const [customerMin,setCustomerMin]=useState(''); const [customerMax,setCustomerMax]=useState(''); const [providerBudget,setProviderBudget]=useState('');
  const [local,setLocal]=useState({evidence_kind:'provider_quote',category:'Handyman',sub_service:'',geography:'Harish',customer_price:'',provider_amount:'',scope_notes:''});
  const load=async()=>{try{const [r,e,l,b]=await Promise.all([cleanfixApi.listPricingReferences(),cleanfixApi.listPriceEstimates(),cleanfixApi.listLocalPriceEvidence(),cleanfixApi.getLocalPriceBenchmarks()]);setReferences(r.items||[]);setEstimates(e.items||[]);setLocalEvidence(l.items||[]);setBenchmarks(b.items||[]);}catch{toast.error('Pricing workspace could not load.');}};
  useEffect(()=>{load();},[]);
  const reference=references.find((item)=>String(item.id)===selected);
  const create=async()=>{if(!reference||description.trim().length<10){toast.error('Choose verified evidence and describe the job clearly.');return;}try{await cleanfixApi.createPriceEstimate({lead_id:leadId==='none'?null:Number(leadId),observation_id:reference.id,service_description:description,geography,customer_min:customerMin?Number(customerMin):null,customer_max:customerMax?Number(customerMax):null,provider_budget:providerBudget?Number(providerBudget):null});toast.success('Draft saved. It has not been sent to the customer.');setDescription('');await load();}catch(error:any){toast.error(error?.response?.data?.detail||'Draft was not saved.');}};
  const approve=async(id:number)=>{try{await cleanfixApi.approvePriceEstimate(id);toast.success('Owner approval recorded. Nothing was sent automatically.');await load();}catch(error:any){toast.error(error?.response?.data?.detail||'Approval failed.');}};
  const addEvidence=async()=>{if(local.sub_service.trim().length<2||local.scope_notes.trim().length<10){toast.error('Add the service and exact scope.');return;}try{await cleanfixApi.createLocalPriceEvidence({...local,customer_price:local.customer_price?Number(local.customer_price):null,provider_amount:local.provider_amount?Number(local.provider_amount):null});toast.success('Local evidence saved for owner review.');setLocal({...local,sub_service:'',customer_price:'',provider_amount:'',scope_notes:''});await load();}catch{toast.error('Local evidence was not saved.');}};
  const approveEvidence=async(id:number)=>{try{await cleanfixApi.approveLocalPriceEvidence(id);toast.success('Local evidence approved and counted.');await load();}catch{toast.error('Local evidence approval failed.');}};
  return <><SectionTitle eyebrow="Evidence-backed pricing" title="Price estimator" description="Use verified national references, add the real job scope, then approve the price yourself. The system never sends or finalizes a price automatically."/>
    <div className="grid gap-6 xl:grid-cols-2"><Panel title="1. Create a draft estimate" subtitle="Only green Verified rows can be used">
      <Label>Market reference</Label><Select value={selected} onValueChange={setSelected}><SelectTrigger className="mt-2 bg-white"><SelectValue placeholder="Choose a verified service"/></SelectTrigger><SelectContent>{references.filter(r=>r.eligible_for_estimate).map(r=><SelectItem key={r.id} value={String(r.id)}>{r.sub_service} · ₪{r.min_price}–₪{r.max_price}</SelectItem>)}</SelectContent></Select>
      {reference&&<div className="mt-3 rounded-xl bg-[#E4ECEA] p-3 text-xs text-[#31585E]"><strong>{reference.source.publisher}</strong> · National reference · {reference.vat_status}<br/><a className="underline" href={reference.source.url} target="_blank" rel="noreferrer">Open source</a></div>}
      <div className="mt-4 grid gap-3 sm:grid-cols-2"><div><Label>Customer</Label><Select value={leadId} onValueChange={setLeadId}><SelectTrigger className="mt-1 bg-white"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="none">No customer selected</SelectItem>{leads.map(l=><SelectItem key={l.id} value={String(l.id)}>{l.customerName}</SelectItem>)}</SelectContent></Select></div><FieldInput label="Area" value={geography} onChange={setGeography}/></div>
      <div className="mt-3"><FieldArea label="Exact work requested, measurements, access and photo findings" value={description} onChange={setDescription} large/></div>
      <div className="grid gap-3 sm:grid-cols-3"><FieldInput label="Customer minimum ₪" value={customerMin} onChange={setCustomerMin}/><FieldInput label="Customer maximum ₪" value={customerMax} onChange={setCustomerMax}/><FieldInput label="Provider budget ₪" value={providerBudget} onChange={setProviderBudget}/></div>
      <Button onClick={create} className="mt-4 w-full bg-[#174E57]">Save draft for my review</Button><p className="mt-2 text-xs text-[#786F65]">Non-binding. Final scope and price require Aviel’s approval.</p>
    </Panel><Panel title="2. Owner approval queue" subtitle="No customer message is sent from this screen">{estimates.map(e=><div key={e.id} className="mb-3 rounded-2xl border bg-white p-4"><div className="flex justify-between gap-3"><div><p className="text-sm font-medium">{e.service_description}</p><p className="mt-1 text-xs text-[#786F65]">{e.geography} · Market ₪{e.suggested_min}–₪{e.suggested_max}</p><p className="text-xs text-[#786F65]">Customer ₪{e.customer_min||'—'}–₪{e.customer_max||'—'} · Provider budget ₪{e.provider_budget||'—'}</p></div><Badge className={e.status==='approved'?'bg-[#DCEADF] text-[#2E6840]':'bg-[#EEE4D4] text-[#765D38]'}>{e.status}</Badge></div>{e.status==='draft'&&<Button size="sm" className="mt-3 bg-[#174E57]" onClick={()=>approve(e.id)}><Check className="mr-1 h-4 w-4"/>Approve</Button>}</div>)}{!estimates.length&&<EmptyState text="No estimates yet."/>}</Panel>
      <Panel title="3. Record real local evidence" subtitle="Provider quote or completed CleanFixHarish job"><div className="grid gap-3 sm:grid-cols-2"><div><Label>Evidence type</Label><Select value={local.evidence_kind} onValueChange={v=>setLocal({...local,evidence_kind:v})}><SelectTrigger className="mt-1 bg-white"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="provider_quote">Provider quote</SelectItem><SelectItem value="completed_job">Completed job</SelectItem></SelectContent></Select></div><FieldInput label="Area" value={local.geography} onChange={v=>setLocal({...local,geography:v})}/><FieldInput label="Category" value={local.category} onChange={v=>setLocal({...local,category:v})}/><FieldInput label="Specific service" value={local.sub_service} onChange={v=>setLocal({...local,sub_service:v})}/><FieldInput label="Customer price ₪" value={local.customer_price} onChange={v=>setLocal({...local,customer_price:v})}/><FieldInput label="Provider amount ₪" value={local.provider_amount} onChange={v=>setLocal({...local,provider_amount:v})}/></div><div className="mt-3"><FieldArea label="Exact comparable scope" value={local.scope_notes} onChange={v=>setLocal({...local,scope_notes:v})} large/></div><Button variant="outline" className="mt-3 w-full" onClick={addEvidence}>Save as pending evidence</Button></Panel>
      <Panel title="4. Review local evidence" subtitle="Only Aviel-approved records count">{localEvidence.map(item=><div key={item.id} className="mb-3 rounded-2xl border bg-white p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-medium">{item.sub_service} · {item.geography}</p><p className="mt-1 text-xs text-[#786F65]">{item.evidence_kind.replace('_',' ')} · Customer ₪{item.customer_price||'—'} · Provider ₪{item.provider_amount||'—'}</p><p className="mt-1 text-xs text-[#786F65]">{item.scope_notes}</p></div><Badge className={item.status==='approved'?'bg-[#DCEADF] text-[#2E6840]':'bg-[#EEE4D4] text-[#765D38]'}>{item.status}</Badge></div>{item.status==='pending'&&<Button size="sm" variant="outline" className="mt-3" onClick={()=>approveEvidence(item.id)}><Check className="mr-1 h-4 w-4"/>Approve evidence</Button>}</div>)}{!localEvidence.length&&<EmptyState text="No local evidence yet."/>}</Panel>
      <Panel title="Evidence status" subtitle="What the estimator is allowed to use"><div className="grid grid-cols-2 gap-3"><Metric label="Verified" value={references.filter(r=>r.eligible_for_estimate).length} note="Usable national rows" icon={BadgeCheck}/><Metric label="Pending" value={references.filter(r=>!r.eligible_for_estimate).length} note="Blocked from estimates" icon={Clock3} tone="brass"/></div><p className="mt-4 text-sm leading-6 text-[#625B53]">Harish and Pardes Hanna adjustments appear only after at least five owner-approved comparable local records with both customer and provider amounts.</p><div className="mt-4 space-y-2">{benchmarks.map(item=><div key={`${item.category}-${item.sub_service}-${item.geography}`} className="rounded-xl border bg-white p-3 text-xs"><div className="flex justify-between gap-2"><strong>{item.sub_service} · {item.geography}</strong><span>{item.sample_count}/{item.minimum_samples} samples</span></div><p className="mt-1 text-[#786F65]">{item.ready_for_guidance?`Customer average ₪${Number(item.customer_average).toFixed(0)} · Provider average ₪${Number(item.provider_average).toFixed(0)} · Retained ₪${Number(item.margin_average).toFixed(0)}`:'Local guidance remains hidden until enough comparable evidence exists.'}</p></div>)}</div></Panel>
    </div></>;
}
