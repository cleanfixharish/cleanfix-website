...
class ServiceQuote(Base):
    ...
    lead_id = Column(Integer, ForeignKey("leads.id"), nullable=True, index=True)
    pilot_task_classification_id = Column(Integer, ForeignKey("pilot_task_classifications.id", ondelete="RESTRICT"), nullable=True, index=True)
    pilot_task_key = Column(String(60), nullable=True)
    pilot_scope_hash = Column(String(64), nullable=True)
